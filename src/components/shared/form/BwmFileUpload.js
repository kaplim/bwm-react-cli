...
export class BwmFileUpload extends React.Component {
	...
	setupReader() {
		...
		this.reader.addEventListener('load', (event) => {
			...
			if (initialImageBase64) {
				...
				this.setState({
					imageBase64,
					initialImageBase64: event.target.result
				});
			}
		});
	}
	...
	onImageLoaded(image) {
		...
		if (image.naturalWidth < 950 && image.naturalHeight < 720) {
			...
			toast.error('Minimum width of an image is 950px width and ' +
				'720px height.');
			return;
		}
	...
	async onCropCompleted(crop, pixelCrop) {
		...
		if (selectedFile && (pixelCrop.height > 0 && pixelCrop.width > 0)) {
			...
			const croppedImage =
				await getCroppedImg(img, pixelCrop, selectedFile.name);
	...
	render() {
		...
		const { selectedFile, imageBase64, crop, initialImageBase64 }
			= this.state;

		return (
			<div className="form-group img-upload-container">
